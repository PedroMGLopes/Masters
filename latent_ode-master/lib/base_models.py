...
class Baseline(nn.Module):
	def __init__(self, input_dim, latent_dim, device,
		...
		if use_regression:
			if linear_classifier:
				...
				self.classifier = create_classifier(z0_dim, 1)
			utils.init_network_weights(self.classifier)
	...
	def compute_all_losses(self, batch_dict,
		...
		if (batch_dict["labels"] is not None) and self.use_regression:
				mse_loss = compute_label_mse(info["label_predictions"],batch_dict["labels"],mask = batch_dict["mask_predicted_data"])
		...
		if self.use_binary_classif:
			if self.train_classif_w_reconstr:
				loss = loss +  ce_loss * 400
			else:
				loss =  ce_loss

		if self.use_regression:
			if self.train_classif_w_reconstr:
				loss = loss +  mse_loss * 2000
			else:
				loss =  mse_loss
