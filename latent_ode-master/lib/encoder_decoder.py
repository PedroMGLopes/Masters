...
class Encoder_z0_ODE_RNN(nn.Module):
	...
	def run_odernn(self, data, time_steps,
		run_backwards = True, save_info = False, feed_previous=False, decoder=None, first_hidden=None):
		# IMPORTANT: assumes that 'data' already has mask concatenated to it
		...
		for i in time_points_iter:
			...
			if not feed_previous:
				...
				if i == 0:
					...
					xi=decoder(yi_ode)
					maskxi1=torch.zeros(xi.size()).to(xi.get_device())
					maskxi2=torch.ones(xi.size()).to(xi.get_device())
					xi = torch.cat([xi,maskxi2],-1)
