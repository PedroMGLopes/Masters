...
def compute_multiclass_CE_loss(label_predictions, true_label, mask):
	...
	for i in range(true_label.size(0)):
		pred_masked = torch.masked_select(label_predictions[i], pred_mask[i].to(torch.bool))
		labels = torch.masked_select(true_label[i], label_mask[i].to(torch.bool))
...
def compute_label_mse(label_predictions, true_label,mask):
	...
	# For each trajectory, we get n_traj_samples samples from z0 -- compute loss on all of them
	true_label = true_label.repeat(n_traj_samples, 1, 1, 1)
	mask = mask.repeat(n_traj_samples, 1, 1, 1)

	true_label_masked=torch.masked_select(true_label,mask.to(torch.bool))
	label_predictions_masked=torch.masked_select(label_predictions,mask.to(torch.bool))

	mselossfn=nn.MSELoss()
	mse_loss=mselossfn(label_predictions_masked,true_label_masked)
	return mse_loss


def compute_label_mse_vae(label_predictions, true_label,mask):
	#print("Computing multi-class classification loss: compute_multiclass_CE_loss")
	...
	true_label_masked=torch.zeros(n_traj_samples,n_traj,1)
	label_predictions_masked=torch.zeros(n_traj_samples,n_traj,1)

	for j in range(n_traj):
		for i in range(n_traj_samples):
			true_label_masked[i,j,0]=true_label[i,j,max_t[j],0]
			label_predictions_masked[i,j,0]=label_predictions[i,j,max_t[j],0]
	...
	for i in range(n_traj_samples):
		for k in range(n_traj):
			for j in range(n_dims):
				data_masked = torch.masked_select(data[i,k,:,j], mask[i,k,:,j].to(torch.bool))

				#assert(torch.sum(data_masked == 0.) < 10)

				mu_masked = torch.masked_select(mu[i,k,:,j], mask[i,k,:,j].to(torch.bool))
