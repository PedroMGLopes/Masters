...
class ODE_RNN(Baseline):
	def __init__(self, input_dim, latent_dim, device = torch.device("cpu"),
		...
		concat_mask = False, obsrv_std = 0.1, use_binary_classif = False, use_regression = False,
	...
	def forwardextrap(self, time_steps_to_predict, initial_hidden,last_cell_output, mask = None, n_traj_samples = 1, mode = None):
		batch_size=initial_hidden.size(0)
		last_cell_output=last_cell_output.repeat(1,time_steps_to_predict.size(0),1)
		data_and_mask = last_cell_output

		mask1=torch.zeros(last_cell_output.size()).to(last_cell_output.get_device())
		mask2=torch.zeros(last_cell_output.size()).to(last_cell_output.get_device())
		data_and_mask = torch.cat([mask1, mask2],-1)

		_, _, latent_ys, _ = self.ode_gru.run_odernn(
			data_and_mask, time_steps_to_predict, run_backwards = False, feed_previous = True, decoder=self.decoder, first_hidden=initial_hidden)
		...
		first_point = last_cell_output[:,0,:]
	...
	def get_reconstruction(self, time_steps_to_predict, data, truth_time_steps,
		...
		extra_info["last_hidden"]=last_hidden
