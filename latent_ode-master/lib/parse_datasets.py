...
from physiosepsis import PhysioSepsis, variable_time_collate_fn_sepsis, get_data_min_max, clone_subsequences
...
def parse_datasets(args, device):
	...
	if dataset_name == "physiosepsis":
		dataset_obj_a = PhysioSepsis('data/physiosepsis', train=True,
										quantization = args.quantization,
										download=True, n_samples = args.n,
										device = device)
		print('done importing A')
		# Use custom collate_fn to combine samples with arbitrary time observations.
		# Returns the dataset along with mask and time steps
		dataset_obj_b = PhysioSepsis('data/physiosepsis', train=False,
										quantization = args.quantization,
										download=True, n_samples = args.n,
										device = device)
		print('done importing B')
		# Combine and shuffle samples from physionet Train and physionet Test
		total_dataset = dataset_obj_a[:len(dataset_obj_a)]+ dataset_obj_b[:len(dataset_obj_b)]
		data_min, data_max = get_data_min_max(total_dataset)
		train_dataset = dataset_obj_a[:math.floor(len(dataset_obj_a)/2)]+ dataset_obj_b[:math.floor(len(dataset_obj_b)/2)]
		test_dataset=dataset_obj_a[math.floor(len(dataset_obj_a)/2):]+ dataset_obj_b[:math.floor(len(dataset_obj_b)/2):]
		if args.load is None:
			train_data, test_data = model_selection.train_test_split(train_dataset, train_size= 0.8,random_state = 42, shuffle = True)
			i=0
			train_data=train_data[:args.n*3]
			while i<len(train_data):
				if not np.any(train_data[i][-1].cpu().detach().numpy()>0):
					if random.random()>0.55:
						train_data.pop(i)
				i=i+1
			train_data=train_data[:math.floor(args.n*0.8)]
			test_data=train_data[:math.floor(args.n*0.2)]



		print('done train test split')
		if((args.latent_ode or args.rnn_vae) and args.load is None):
			train_data=clone_subsequences(train_data,all_subsequences=True)
		...
		if args.load is None:
			train_dataloader = DataLoader(train_data, batch_size= batch_size, shuffle=False,
				collate_fn= lambda batch: variable_time_collate_fn_sepsis(batch, args, device, data_type = "train",
					data_min = data_min, data_max = data_max))

			test_dataloader = DataLoader(test_data, batch_size = batch_size*5, shuffle=False,
				collate_fn= lambda batch: variable_time_collate_fn_sepsis(batch, args, device, data_type = "test",
				data_min = data_min, data_max = data_max))

		if args.load is not None and not (args.interp_test or args.extrap_test):
			test_dataset=test_dataset[:args.n]
			if(args.latent_ode or args.rnn_vae):
				test_sepsis=clone_subsequences(test_dataset,all_subsequences=True)
			else:
				test_sepsis=test_dataset
			test_sepsis_dataloader=DataLoader(test_sepsis, batch_size = batch_size*5, shuffle=False,
				collate_fn= lambda batch: variable_time_collate_fn_sepsis(batch, args, device, data_type = "test",
					data_min = data_min, data_max = data_max))

		elif args.load is not None and (args.interp_test or args.extrap_test):
			test_dataset=test_dataset[:args.n]
			problem_test_dataloader=DataLoader(test_dataset, batch_size = batch_size, shuffle=False,
				collate_fn= lambda batch: variable_time_collate_fn_sepsis(batch, args, device, data_type = "test",
					data_min = data_min, data_max = data_max))

		print('done test loader')
		attr_names = dataset_obj_a.params
		if args.load is not None and not (args.interp_test or args.extrap_test):
			data_objects = {"dataset_obj": [test_dataset,test_sepsis],
						"test_sepsis_dataloader":utils.inf_generator(test_sepsis_dataloader),
						"input_dim": input_dim,
						"n_test_sepsis_batches": len(test_sepsis_dataloader),
			...
			data_objects = {"dataset_obj": [test_dataset],
						"problem_test_dataloader": utils.inf_generator(problem_test_dataloader),
						"input_dim": input_dim,
						"n_test_batches": len(problem_test_dataloader),
						"attr": attr_names, #optional
						"classif_per_tp": True, #optional
						"n_labels": 1,
						"data_min": data_min,
			...
			data_objects = {"dataset_obj": [train_data,test_data],
						"train_dataloader": utils.inf_generator(train_dataloader),
						"test_dataloader": utils.inf_generator(test_dataloader),
						"input_dim": input_dim,
						"n_train_batches": len(train_dataloader),
